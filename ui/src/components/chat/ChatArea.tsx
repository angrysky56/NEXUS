...
import { Send, User, Bot, Paperclip, Settings as SettingsIcon } from 'lucide-react';
...
export const ChatArea: React.FC<ChatAreaProps> = ({ sessionId }) => {
    ...
    useEffect(() => {
        api.fetchModels().then(fetchedModels => {
    ...
    useEffect(() => {
        if (currentModel) {
            localStorage.setItem('NEXUS_SELECTED_MODEL', currentModel);
        }
    }, [currentModel]);
    ...
    const handleSend = async () => {
        ...
        // Metadata Check: Reasoning Support
        const supportsReasoning = modelInfo?.supported_parameters?.some((p: string) => p === 'reasoning' || p === 'include_reasoning') || false;

        // Adaptive Limits Check (Simple warning for now)
        if (modelInfo?.context_length) {
            const estimatedTokens = input.length / 4;
            if (estimatedTokens > modelInfo.context_length) {
                console.warn("Input exceeds model context length");
                // Could toast or block here, but for MVP just warn
            }
        }
            ...
            api.streamChat({
                ...
                if (event.type === 'usage') {
                    setUsageStats(prev => {
                        ...
                        const inputCost = (event.prompt_tokens * parseFloat(pricing.prompt)) || 0;
                        const outputCost = (event.completion_tokens * parseFloat(pricing.completion)) || 0;
                        // Reasoning tokens are usually included in total completion tokens for billing

                        return {
                            inputTokens: prev.inputTokens + event.prompt_tokens,
                            outputTokens: prev.outputTokens + event.completion_tokens,
                            reasoningTokens: prev.reasoningTokens + (event.reasoning_tokens || 0),
                ...
                setMessages(prev => {
                    ...
                    if (event.type === 'token') {
                        ...
                        for (const tc of toolCalls) {
                            ...
                            const existingIdx = existingToolCalls.findIndex((t: any) => t.index === idx);
                        ...
                        const tc = (lastMsg.toolCalls || []).find((t: any) => t.id === event.id);
        ...
        <div className="flex flex-col h-full bg-slate-950">
            ...
            <div className="h-14 border-b border-slate-800 flex items-center justify-between px-4 bg-slate-900/50 backdrop-blur-sm relative z-20">
                <div className="flex items-center gap-3">
                    <ModelSelector models={models} currentModel={currentModel} onSelect={setCurrentModel} />
                    ...
                    <div className="hidden md:flex items-center gap-4 text-[10px] text-slate-500 font-mono ml-4 border-l border-slate-700 pl-4">
                        <div className="flex flex-col">
                           ...
                           <span>{usageStats.inputTokens} / {usageStats.outputTokens}</span>
                        </div>
                        {usageStats.reasoningTokens > 0 && (
                            <div className="flex flex-col text-neon-purple">
                                <span className="opacity-70">REASONING</span>
                                <span className="text-neon-purple">{usageStats.reasoningTokens}</span>
                            </div>
                        )}
