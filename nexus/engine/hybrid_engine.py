...
class HybridEngine(BicameralEngine):
    ...
    async def process_stream(
        ...
        logger.info(
            f"[ENGINE] Processing started - history_len={len(history)}, tools_count={len(tools or [])}, reasoning={include_reasoning}"
        )
        logger.debug(
            f"[ENGINE] Input preview: {user_input[:100]}..."
            if len(user_input) > 100
            else f"[ENGINE] Input: {user_input}"
        )
        ...
        try:
            current_embedding = await self.client.get_embeddings(
        ...
        logger.debug(
            f"[ENGINE] Trajectory matrix: shape={trajectory.shape}, embedding_history={len(self.history_embeddings)}"
        )
        ...
        logger.info(
            f"[ENGINE] Routing result: ID={routing.intrinsic_dimension:.3f}, gate={routing.gate_value:.3f}, manifold={routing.primary_manifold.value}"
        )
        ...
        logger.debug(
            f"[ENGINE] Target emotional state: valence={target_state.valence:.2f}, arousal={target_state.arousal:.2f}"
        )

        # Smoothly transition to target state
        current_emotional_state = self.pid_controller.compute(target_state)
        logger.info(
            f"[ENGINE] PID output: valence={current_emotional_state.valence:.3f}, arousal={current_emotional_state.arousal:.3f}"
        )
        ...
        if await self.client.supports_parameter(selected_model, "temperature"):
            ...
            temperature = (
                await self.client.get_model_default_temp(selected_model) or 1.0
            )

        # High visibility log for verification
        logger.info(
            f"\n[NEXUS ROUTING] *** ACTIVE MANIFOLD: {routing.primary_manifold.value.upper()} ***"
        )
        logger.info(
            f"[ENGINE] Selected: manifold={'LOGIC' if is_logic else 'CREATIVE'}, model={selected_model}, temp={temperature}"
        )

        # 5. GENERATE DYNAMIC SELF-AWARENESS CONTEXT
        # This injects real-time cognitive state into the system prompt
        logger.info(
            "[ENGINE] Stage 5/6: SELF-AWARENESS CONTEXT - Generating system prompt..."
        )
        ...
        yield {
            ...
            "data": {
                **cognitive_state,
                "system_prompt": system_context,  # Pass full context for loop continuation
            },
        }
        ...
        chunk_count = 0
        async for chunk in target_processor.process_chat(
            history,
            user_input,
            tools,
            api_key=api_key,
            ...
            if chunk.get("type") == "tool_call_chunk":
                ...
                logger.debug(
                    f"[ENGINE] Tool call chunk: idx={tc.get('index')}, name={tc.get('function', {}).get('name', '...')}"
                )
            yield chunk
